...
void solve()
{
    ...
    auto merge = [&](DNA &a, DNA b, DNA c) // c更右
    {
    ...
    auto lca = [&](int x, int y) -> std::pair<int, int> {
        if (node[x].depth < node[y].depth)
            swap(x, y);
        // cout<<x<<y<<endl;
        for (int i = __lg(node[x].depth); i >= 0; i--)
        {
            if (node[node[x].dna[i].fa].depth >= node[y].depth)
                x = node[x].dna[i].fa;
        }
        if (x == y)
    ...
    auto check = [&](int x, int y) {
        ...
        DNA an;
        int len = x;
        int si = node[x].depth - node[y].depth + 1;
        for (int i = __lg(node[x].depth - node[y].depth + 1); i >= 0; i--)
        {
            if (si > 0 && node[len].depth - node[y].depth + 1 >= (1 << i))
            {
                merge(an, node[len].dna[i], an);
                si -= (1 << i);
    ...
    while (q--)
    {
        ...
        if (flag == '+')
        {
            ...
            node[si].dna[0].fa = v;
            for (int k = 1; k <= __lg(node[si].depth + 1); k++)
            {
                int fath = node[si].dna[k - 1].fa;
                node[si].dna[k].fa = node[fath].dna[k - 1].fa;
                merge(node[si].dna[k], node[fath].dna[k - 1], node[si].dna[k - 1]);
            }
        }
        else
        {
            ...
            else
            {
                if (node[u].depth < node[v].depth)
                    swap(u, v);
                DNA te;
                auto an = check(u, tem.second);
                swap(an.lw, an.rw), swap(an.lwm, an.rwm);
                auto an1 = check(v, tem.first);
                merge(te, an, an1);
                if (te.w >= k && te.wm <= k)
                {
                    cout << "YES" << endl;
