...
const i64 M = 23333333333333333ll;
struct DSU
{
...
int main()
{
    ...
    while (t--)
    {
        ...
        for (auto &i : a)
            cin >> i, i--;
        sort(a.begin(), a.end(), [&](int x, int y) { return sa.rk[x] < sa.rk[y]; });
        a.erase(unique(a.begin(), a.end()), a.end());
        i64 ans = 0;
        vector<int> w;
        for (int i = 1; i < a.size(); i++)
        {
            w.push_back(ask(a[i], a[i - 1]));
        }
        vector<int> o(w.size());
        ...
        {
            ans += (1ll * f.size(o[i]) * f.size(o[i] + 1) % M) * w[o[i]] % M;
            f.merge(o[i], o[i] + 1);
            ans %= M;
        }
        cout << ans << endl;
