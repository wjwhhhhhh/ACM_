...
struct Point_divide:UNdigraph //点分治
{
    ...
    void calc(int u)
    {
        ...
        for(int i=h[u];~i;i=ne[i])
        {
            ...
            while(q.size())
            {
                //if(u==6)cerr<<q.front()<<endl;
                //if(u==6)cerr<<ans.size()<<endl;
                p.push(q.front());
                ans[q.front()]++;
                y._a[q.front()]++;
                q.pop();
            ...
            if(u==6)
            {
                for(auto ch:x._a)cerr<<ch<<" ";
                cerr<<endl;
            }
            for(int i=1;i<c._a.size()&&i<ans.size();i++)ans[i]+=c._a[i];
            while(p.size())
            {
                x._a[p.front()]++;
                y._a[p.front()]--;
