...
struct Matrix
{
    ...
    Matrix(i64 n, i64 m)
    {
        A = std::vector(n, std::vector<i64>(n, 0));
        B = std::vector(n, std::vector<i64>(n, 0));
        M = m, W = n;
    }
    void mul(vector<vector<i64>> &ans, vector<vector<i64>> A, vector<vector<i64>> B)
    {
        for (int i = 0; i < W; i++)
...
template <class T> inline int read()
{
    char c = getchar();
    int x = 0, s = 1;
    while (c < '0' || c > '9')
