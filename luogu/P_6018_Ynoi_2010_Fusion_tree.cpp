...
int main()
{
    ...
    while (m--)
    {
        ...
        if (opt == 1)
        {
            ...
            {
                if (fa[fa[x]] != -1)
                    tire.erase(get(fa[x]), tire.rt[fa[fa[x]]], 0);
                v[fa[x]]++;
                if (fa[fa[x]] != -1)
                    tire.insert(get(fa[x]), tire.rt[fa[fa[x]]], 0); // 重新插入
            }
            tire.addall(tire.rt[x]); // 对所有节点+1
        }
        else if (opt == 2)
        {
            int w;
            cin >> w;
            if (x != root)
                tire.erase(get(x), tire.rt[fa[x]], 0);
            v[x] -= w;
            if (x != root)
                tire.insert(get(x), tire.rt[fa[x]], 0); // 重新插入
        }
        else
        {
            int ans = 0;
